...
CGeoPoint CIndirectDefender::generatePos(const CVisionModule* pVision)
{
	...
	if (_mode == 1)
	{
		...
		//////////////////////////////////////////////////////////////////////////
		//先跑这个
		if (/*"theirIndirectKick" == refMsg || *//*"theirDirectKick" == refMsg*/0)
		{
			const CGeoLine RrealDefendLine = DefendUtils::getDefenceTargetAndLine(_RdefendTarget,_RdefendDir);
			const CVector elseVector = Utils::Polar2Vector(RADIUS,Utils::Normalize(_RdefendDir + Param::Math::PI));
			defendPos = ballPos + elseVector;
		} else if (Utils::OutOfField(defendPos,FIELD_BUFFER) || enemy.Pos().dist(ballPos) > Param::Field::FREE_KICK_AVOID_BALL_DIST)
		{
