...
//const double VELOCITY_THRES = 50;
...
FilteredObject::FilteredObject(double A[2][2], double B[2], double C[2][2], double D[2],double lossMat[2])
{
...
void FilteredObject::initialize(const std::string& filename1, const std::string& filename2)
{
  // open the file 1
  ifstream matFile(filename1.c_str());
  if (matFile.is_open())
  {
	...
	matFile>>VELOCITY_THRES;                  //  进行修正的阀值
    // close the file
    matFile.close();
    usageCount = 0;
    ...
    double determinant = 1.0f/(Cimp[0][0]*Cimp[1][1]-Cimp[0][1]*Cimp[1][0]);
  ...
  // open the file 2
  ifstream matFile2(filename2.c_str());
  if (matFile2.is_open())
  {
...
void FilteredObject::initializePos(double pos[2])
{
...
void FilteredObject::updatePosition(double z[2], double filtOut[2][2])
{
  ...
  ************************************************/
  ...
  // x-axis
  if (abs(filtOut[0][1]-filtOutP[0][1])>VELOCITY_THRES)
  {
    // x-axis
  ...
  if (abs(filtOut[1][1]-filtOutP[1][1])>VELOCITY_THRES)
  { 
    // y-axis
  ...
  ************************************************/
  ...
  double tempfiltState[2][2]; double tempfiltStateP[2][2];
