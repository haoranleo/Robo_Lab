...
string CBallStatus::checkBallState(const CVisionModule* pVision,int meNum){
	...
	double heSpeedInBallVel=heSpeed*cos(Utils::Normalize(he.Vel().dir()-ball.Vel().dir()));
	...
	bool isOurBall=ball.Valid()&&
		(isBallMovtoMe && notHeGetBallBefore
			...
			&&(balltoHeDist-balltoMeDist>judgeRange || balltoHeDist>balltoMeDist+5 && isBallInFront))
		);
	//waitTouch
