...
CAttributeFactory::CAttributeFactory()
{
	...
	LEFTGUDGE = CGeoPoint(-Param::Field::PITCH_LENGTH/2,-160*Param::Field::RATIO);
	RIGHTGUDGE = CGeoPoint(-Param::Field::PITCH_LENGTH/2,160*Param::Field::RATIO);
...
EVALUATE_ATTRIBUTE(ACanBeLeader)
{
	...
	if (num == BestPlayer::Instance()->getTheirBestPlayer())
	{
		...
		double toBallDir = (pVision->Ball().Pos() - pVision->TheirPlayer(num).Pos()).dir();
		//cout<<"best wrong direction "<<toBallDir*180.0/Param::Math::PI<<" "<<(toBallDir<Param::Math::PI/2)<<" "<<(toBallDir> -Param::Math::PI/2)<<endl;
		if ((("theirIndirectKick" == refMsg || "theirDirectKick" == refMsg) && pVision->Ball().X()>160) && toBallDir<Param::Math::PI/2 && toBallDir> -Param::Math::PI/2){
			for (int i = 1;i <= Param::Field::MAX_PLAYER; i++)
			{
				if (i!=num)
				{
					...
					{
						CGeoPoint he = pVision->TheirPlayer(i).Pos();
						//GDebugEngine::Instance()->gui_debug_line(he,he+Utils::Polar2Vector(500,toLeftGudgeDir),COLOR_WHITE);
						//GDebugEngine::Instance()->gui_debug_line(he,he+Utils::Polar2Vector(500,toRightGudgeDir),COLOR_WHITE);
						//GDebugEngine::Instance()->gui_debug_line(he,he+Utils::Polar2Vector(500,othertoBallDir),COLOR_WHITE);
						//cout<<"best wrong direction "<<othertoBallDir*180.0/Param::Math::PI<<" "<<(othertoBallDir>=toRightGudgeDir)<<" "<<(othertoBallDir<=toLeftGudgeDir)<<endl;
					}
				
					if (pVision->TheirPlayer(i).Pos().dist(pVision->Ball().Pos())<50 && othertoBallDir>=toRightGudgeDir && othertoBallDir<=toLeftGudgeDir)
					{
						tempValue = 0;
					}
				}
	...
	}else if (num != BestPlayer::Instance()->getTheirBestPlayer())
	{
		...
		if (("theirIndirectKick" == refMsg || "theirDirectKick" == refMsg) && besttoBallDir<Param::Math::PI/2 && besttoBallDir> -Param::Math::PI/2)
		{
			...
			if (pVision->TheirPlayer(num).Pos().dist(pVision->Ball().Pos())<50 &&  metoBallDir>=toRightGudgeDir && metoBallDir<=toLeftGudgeDir)
			{
				tempValue = 1;
			}
		}
...
EVALUATE_ATTRIBUTE(AGoalie)
{
	...
	if (num == goalieNum && pVision->TheirPlayer(num).Pos().x() > 200)
	{
		tempValue = 1.0;
	}
	setValue(tempValue);
...
EVALUATE_ATTRIBUTE(ATouchAbility)
{
	...
	if (ballVelMod > OPP_PASS_VEL)
	{
		...
		if (angleDiff < Param::Math::PI * 0.6 && oppPos.x() < 150)//0.6为可以touch的角度极限
		{
