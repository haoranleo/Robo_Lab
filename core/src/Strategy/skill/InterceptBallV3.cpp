...
void CInterceptBallV3::planApproachOpp(const CVisionModule* pVision, const int myID, const int oppID,const double finaldir,double subDist)
{
	...
	bool isPrepare=projDist<5&&Utils::Normalize(me.Dir()-antiBallvel)<Param::Math::PI*6/180;
	...
	if (ball.Vel().mod() <100){
		...
		if (!isPrepare&&ball2projDist/(ball.Vel().mod()-20)<projDist/150+fabs(Utils::Normalize(faceDir-me.Dir()))/(Param::Math::PI*1.8)+0.08){
		...
		if (Utils::OutOfField(predictPoint,10)){
			CGeoRectangle Field=CGeoRectangle(CGeoPoint(302,-202),CGeoPoint(-302,202));
