...
extern "C" int Skill_ShootBall(lua_State *L)
{
	...
	CPlayerTask* pTask = PlayerRole::makeItShootBall(runner, angle, false, preci, 1200, 1200, flag);
	TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);
	return 0;
}
...
extern "C" int Skill_StaticGetBall(lua_State *L)
{
	int runner = LuaModule::Instance()->GetNumberArgument(1,NULL);
	double angle = LuaModule::Instance()->GetNumberArgument(2,NULL);
	CPlayerTask* pTask = PlayerRole::makeItNoneTrajGetBall(runner, angle, CVector(0,0), 0, 4);
	TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);
	return 0;
}
...
extern "C" int Skill_GoAndTurnKick(lua_State *L)
{
	...
	double angle = LuaModule::Instance()->GetNumberArgument(2,NULL);
	double preci = LuaModule::Instance()->GetNumberArgument(3,NULL);
	CPlayerTask* pTask = PlayerRole::makeItGoAndTurnKick(runner, angle, preci, 0);
	TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);
	return 0;
}
...
extern "C" int Skill_InterKick(lua_State *L)
{
	int runner = LuaModule::Instance()->GetNumberArgument(1,NULL);
	double angle = LuaModule::Instance()->GetNumberArgument(2,NULL);
	CPlayerTask* pTask = PlayerRole::makeItInterKickV3(runner, angle, 0);
	TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);
	return 0;
}
...
extern "C" int Skill_Marking(lua_State *L)
{
	...
	CPlayerTask* pTask = PlayerRole::makeItMarkEnemy(runner, pri, front, flag, CGeoPoint(x,y));
	TaskMediator::Instance()->setPlayerTask(runner, pTask, 1);
	return 0;
}
...
{	
	...
	if (front && vision->TheirPlayer(oppNum).Pos().x()>-180 && false == checkKickOffArea) {
...
extern "C" int FUNC_GetUseSmallField(lua_State* L)
{
	DECLARE_PARAM_READER_BEGIN(General)
	READ_PARAM(USE_SMALL_FIELD)
	DECLARE_PARAM_READER_END

	LuaModule::Instance()->PushBool(USE_SMALL_FIELD);
	return 1;
}
...
luaDef GUIGlue[] = 
{
	...
	{"CGetUseSmallField",	FUNC_GetUseSmallField},
