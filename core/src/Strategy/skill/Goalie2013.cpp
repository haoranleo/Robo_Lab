...
void CGoalie2013::plan(const CVisionModule* pVision)
{
	...
	CGeoPoint taskPoint;
	taskPoint = DefPos2013::Instance()->getDefPos2013(pVision).getGoaliePos();
...
//考虑清球角度
		...
		if (TaskMediator::Instance()->singleBack()==0 && TaskMediator::Instance()->leftBack()!=0){
			CGeoPoint leftpos = DefPos2013::Instance()->getDefPos2013(pVision).getLeftPos();
			CGeoPoint rightpos = DefPos2013::Instance()->getDefPos2013(pVision).getRightPos();
			double goal2leftdir = (DefPos2013::Instance()->getDefPos2013(pVision).getLeftPos() - ball.Pos()).dir() - 0.3;
			double goal2rightdir =  (DefPos2013::Instance()->getDefPos2013(pVision).getRightPos() - ball.Pos()).dir() + 0.3;
			...
			if (Utils::AngleBetween(clearBallDir,goal2leftdir,goal2rightdir)){
				/*cout<<"clearBallDir is "<<clearBallDir*180/Param::Math::PI<<endl;
		...
		{
			double goal2singledir = (DefPos2013::Instance()->getDefPos2013(pVision).getSinglePos() - ball.Pos()).dir();
		...
		if (ball.Pos().dist(goalLeft) < 20 || ball.Pos().dist(goalRight)< 20 || (pVision->TheirPlayer(oppnum).Pos().dist(ball.Pos())<me.Pos().dist(ball.Pos())+100)){
			clearBallDir = CVector(ball.Pos() - goalCenter).dir();
		}*/
...
bool	CGoalie2013::needtoClearBall(const CVisionModule* pVision)
{
	...
	if (((ball.Vel().mod() < 150 && Utils::InOurPenaltyArea(ball.Pos(),-30))||
		...
		//cout<<pVision->Cycle()<<" need to Clear Ball "<<ball.Vel().mod()<<endl;
	}
	return result;
}
...
bool CGoalie2013::needtoAttackEnemy(const CVisionModule* pVision)
{
	...
	if ((Utils::InOurPenaltyArea(enemy.Pos(),DEFEND_PENALTY_BUFFER) && ball.Vel().mod()>120 &&  DefendUtils::BallIsToPenaltyArea()
		&& !DefendUtils::BallIsInShootingTarget()) 
		...
		//cout<<"need to attack enemy"<<endl;
	}
	return result;
}
