...
void CChaseKickV2::plan(const CVisionModule* pVision)
{
	...
	if (ballSpeed>=Ball_Moving_Fast_Speed)
	{
		...
		case SPEED_UP:
			...
			//cout<<"speed Up"<<endl;
			speedUpDistanceY=fabs(kickPos.y()-me.Pos().y())+crossWiseFactor[realNum]*Param::Vehicle::V2::PLAYER_SIZE+max((ballSpeed-100)*sin(fabs(ballVelDir-Param::Math::PI*20/180))*0.25,0);
			...
			if (fabs(ballVelDir)>Param::Math::PI*65/180){
				...
				{
					speedUpDistanceX=(me.Pos().x()-kickPos.x())/2;
				}else{
					speedUpDistanceX=-3;
				}
				finalKickDir=0;
		...
		/************************************************************************/
		if (NormalPlayUtils::faceTheirGoal(pVision,robotNum,Param::Math::PI*5/180))
		{
