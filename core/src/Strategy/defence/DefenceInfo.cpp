...
namespace{
	bool DEFENCE_DEBUG_MODE = false;
	bool ATTACK_DEF_MODE = false;
	bool GOALIE_EVALUATE = false;
...
CDefenceInfo::CDefenceInfo(){
	...
	READ_PARAM(ATTACK_DEF_MODE)
	READ_PARAM(GOALIE_EVALUATE)
...
void CDefenceInfo::updateDefenceInfo(const CVisionModule *pVision){
	...
	if (DEFENCE_DEBUG_MODE)
	{
		...
		for(int i = 1;i<=Param::Field::MAX_PLAYER;++i)
		{
			...
			GDebugEngine::Instance()->gui_debug_msg(playerPos,_oplayer[i]->getRoleName().c_str(),COLOR_YELLOW);
		...
		//cout << "    "<< ut.time()<<endl;
		//cout << "steadyattack array is ";
		for(int i = 1;i<=Param::Field::MAX_PLAYER;++i)
		{
			//cout << _steadyAttackArray[i-1];
		}
		//cout<<endl;
	}
}
	...
	if (0 != attackerTempList.size())
	{
		...
		if(ATTACK_DEF_MODE){
			...
			if (GOALIE_EVALUATE == true){
				int goalieNum = vision->GetTheirGoalieStrategyNum();
				//int goalieNum = 3;
				for(AttackerList::iterator tt = attackerTempList.begin();tt != attackerTempList.end();){
					if (tt->num == goalieNum){
						tt = attackerTempList.erase(tt);
