...
namespace {
	...
	CGeoPoint leftPos       = CGeoPoint(-292.0, -20);
	CGeoPoint rightPos      = CGeoPoint(-292.0, -2);
...
void CPenaltyDefV2::plan(const CVisionModule* pVision) {
	...
	static CGeoPoint RandomPos = CGeoPoint(-292.0, 0);
	...
	case CATEGORY_1:
		{
			_state = RANDOM;
			if (VERBOSE_MODE) { cout << "CATEGORY_1 --> RANDOM" << endl; }
		}
		break;
	case RANDOM:
		{
			if (randomCnt < (rand() % 10 + 6) ) {
				randomCnt++;
			}
			else {
				int y = rand() % 42 - 21;
				//cout << "y: " << y << endl;
				RandomPos = CGeoPoint(self_x, y);
				randomCnt = 0;
			}
		}
		break;
