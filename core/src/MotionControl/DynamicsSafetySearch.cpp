...
#define ACCURATE_AOID 250
	double TEAMMATE_AVOID_DIST = Param::AvoidDist::TEAMMATE_AVOID_DIST / 2; // 厘米 12
	double OPP_AVOID_DIST = Param::AvoidDist::OPP_AVOID_DIST; // 厘米 12
...
CDynamicSafetySearch::CDynamicSafetySearch() {
	...
	_DMAX = MAX_TRANSLATION_DEC;  //小车最大的减速度
	_AMAX = MAX_TRANSLATION_ACC;  //小车最大的加速度
...
float CDynamicSafetySearch::Evaluation(const vector2f tempAcc, const int player) { // 计算权值函数
	...
	val1 = val1 * val1 + 0.5f * (PI * PI - val1 * val1) / (_AMAX * _AMAX) * fabs(best_acc.sqlength() - tempAcc.sqlength());
	val1 = min(val1, PI * PI);
	val2 = fabs(angle_all(last_acc[player - 1].x, last_acc[player - 1].y) - angle_all(tempAcc.x, tempAcc.y));
	if (val2 > PI) {
		val2 = 2 * PI - val2;
	}
	val2 = val2 * val2 + 0.3f * (PI * PI - val2 * val2) / (_AMAX * _AMAX) * fabs(_AMAX * _AMAX - tempAcc.sqlength());
	val2 = min(val2, PI * PI);
...
CVector CDynamicSafetySearch::SafetySearch(const int player, CVector Vnext, const CVisionModule* pVision, const int t_priority, const CGeoPoint target, const int flags, const float stoptime, double max_acc) { // 获得下一帧的速度
	...
	_CNum = max(int(_vel.length() / _AMAX * 60.0f), 15); // 对于一车静止，一车运动的情况，提前量比较少 // 保证下一个周期内不会碰撞
	_C = 1.0f / 60.0f; // 程序执行一个周期的时间
	_acc = (_nvel - _vel) / _C;
	_gamma = _C * _CNum;
	_e = 1.0f;		 // 权值初始化
	...
	if (priority[player - 1] == -1 || CheckAccel(player, _acc, pVision, crashed_car_dir, crash_car_num, type, _C) == SAFE) {
		...
		if (find_flag[player - 1] == true && CheckAccel(player, tempAcc, pVision, crashed_car_dir, crash_car_num, type, _C) == SAFE) { // 判断上次的加速度是否可行
			_t_acc = tempAcc;
		...
		//{//特殊处理，当两个小车的距离很近时，并且没有获得最优解时，需要在一定距离内寻找安全方向
		//	_t_acc = OutOfTrouble(player,_pos,_acc,(TEAMMATE_AVOID_DIST*5.0f ),pVision) * _AMAX;
	...
	if (find_flag[player - 1] == false) {
		if (_vel.length() / _DMAX > _C) {
			_t_acc = _vel.norm() * (-1.0f) * _DMAX;
...
bool CDynamicSafetySearch::CheckAccel (const int player, vector2f acc, const CVisionModule* pVision, vector2f& crash_car_dir, int& crash_car_num, int& type, const float limitTime) {
	...
	while(!order.empty()) {
		...
		if (order.back().first > player) {
			Aj = (vj - vector2f(lastCycle_teamate.VelX(), lastCycle_teamate.VelY())) / _C; // 更改参数时注意，这个也可能需要改变？
		}
		else { // 车号前面的小车，则需要首先考虑它们的加速度
...
bool CDynamicSafetySearch::CheckRobot(const int player, vector2f pi, vector2f vi, vector2f Ai, const int obstacle, vector2f pj, vector2f vj, vector2f Aj, int type, const float limitTime) {
	...
	Pi.MakeTrajectory(player, pi, vi, Ai, 0.0f, _gamma, _stoptime, tradius ,_DMAX, THISPLAYER); // 构建自己的路径
	float tc = min(vj.length() / _DMAX, _C * _CNum);
	if (type == THEIRPLAYER) {
	...
	Pj.MakeTrajectory(obstacle, pj, vj, Aj, 0.0f, tc, Pi._tend, tradius, _DMAX, type); // 构建障碍物的路径
...
vector2f CDynamicSafetySearch::RandomAcc(float start_angle,float end_angle,float max_radius) {
	...
	theta = (float(rand()) / RANDMAX) * (end_angle - start_angle) + start_angle;//产生0~1的随机数
