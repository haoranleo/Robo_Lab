...
CDecisionModule::CDecisionModule(const COptionModule* pOption, CVisionModule* pVision): _pOption(pOption),_pVision(pVision), _playbook(pOption, pVision)
{
...
void CDecisionModule::DoTeamMode()
{
	...
	if (USE_LUA_SCRIPTS){
		...
		play = _playbook.getPlay();
		if (play) {
			play->executePlay(_pOption, _pVision);
		} else {
