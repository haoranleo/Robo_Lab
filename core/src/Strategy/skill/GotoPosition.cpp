...
CPlayerCommand* CGotoPositionV2::execute(const CVisionModule* pVision)
{
	...
	const CVector player2target = target - vecPos;							// 小车到目标的向量
	const double dist = player2target.mod();								// 小车到目标的距离
	...
	double moveDiff = Utils::Normalize(player2target.dir() - self.Dir());	// 朝向和前进方向的差
	...
	if (! isGoalie && fullPath.size() > 5) {	// 非门将球员进行轨迹检查
	...
	if (! fullPathSafe) {	// 检查结果：非门将球员会进入禁区，停车
		if (! isMeInOurPenaltyArea) {
