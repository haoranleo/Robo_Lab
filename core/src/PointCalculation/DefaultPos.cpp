...
bool CDefaultPos::inCourt(int side, int enemyNum){
	bool result = false;
	if (side == -1){
		if (vision->TheirPlayer(enemyNum).Y() < -Param::Field::PITCH_WIDTH/6){
			result = true;
		}
	}else if (side == 0){
		if (vision->TheirPlayer(enemyNum).Y()>= - Param::Field::PITCH_WIDTH/6 && vision->TheirPlayer(enemyNum).Y()<= Param::Field::PITCH_WIDTH/6){
			result = true;
		}
	}else if (side == 1){
		if (vision->TheirPlayer(enemyNum).Y()>Param::Field::PITCH_WIDTH/6){
